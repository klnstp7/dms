...
<table id="buildingBaseDataTable" class="cell-border nowrap" cellspacing="0" width="100%">
</table>
<div class="" style="margin-top: 20px">
    <div class="panel">
        ...
        <div class="panel-body">
            <table id="buildingBaseDataHisTable"  class="cell-border nowrap"  cellspacing="0" width="100%">
            </table>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    var sourceTable;
    <%
    out.print("sourceTable="+SourceTableEmum.BuildingBaseData.ordinal()+";");
    %>
    $(function() {
        _initTable("#buildingBaseDataTable","/buildingBaseData/getPaging");
        $("input").placeholder();
        ...
        $("#rowDown").click(function() {
            if (_table.rows('.selected').data().length == 0) {
                ...
                postcall("/buildingBaseData/batchDownload", {ids: ids}, '_blank');
            }
        });
        ...
        $('#buildingBaseDataTable tbody').on('click', 'tr', function() {
            var _obj = _table.row($(this)).data();
            _initHis("#buildingBaseDataHisTable","/buildingBaseData/getHis",_obj.id);
        });
    ...
    function search(){
        $('#buildingBaseDataTable').DataTable().clear();
        $('#buildingBaseDataTable').DataTable().destroy();
        _initTable("#buildingBaseDataTable","/buildingBaseData/getPaging");
    }

    function info(id, event) {
        stopBubble(event);
        ModifyView("查看楼幢", '/buildingBaseData/add?action=view&id='+id,"720","567");
    }

    function edit(id, event) {
        stopBubble(event);
        ModifyView("修改楼幢", '/buildingBaseData/add?action=edit&id='+id,"720","567");
    }

    function batchDelete(ids) {
        DeletedBatch('批量删除楼幢信息后将无法回复,您确定执行次操作？', '/buildingBaseData/batchDelete',ids);
    }

    function deleted(id, event) {
        stopBubble(event);
        Deleted('楼幢信息删除后将无法回复,您确定执行次操作？','/buildingBaseData/delete',id);
    }
