...
<table id="transactionCaseTable" class="cell-border nowrap"  cellspacing="0" width="100%">
</table>
<div class="" style="margin-top: 20px">
    <div class="panel">
        ...
        <div class="panel-body">
            <table id="transactionCaseHisTable" class="cell-border nowrap" cellspacing="0" width="100%">
            </table>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    var sourceTable;
    <%
    out.print("sourceTable="+SourceTableEmum.TransactionCase.ordinal()+";");
    %>
    $(function() {
        _initTable("#transactionCaseTable","/transactionCase/getPaging");
        $("input").placeholder();
        ...
        $("#rowDown").click(function() {
            if (_table.rows('.selected').data().length == 0) {
                ...
                postcall("/transactionCase/batchDownload", {ids: ids}, '_blank');
            }
        });
        ...
        $('#transactionCaseTable tbody').on('click', 'tr', function() {
            var _obj = _table.row($(this)).data();
            _initHis("#transactionCaseHisTable","/transactionCase/getHis",_obj.id);
        });
    ...
    function search(){
        $('#transactionCaseTable').DataTable().clear();
        $('#transactionCaseTable').DataTable().destroy();
        _initTable("#transactionCaseTable","/transactionCase/getPaging");
    }

    function info(id, event) {
        stopBubble(event);
        ModifyView("查看成交案例", '/transactionCase/add?action=view&id='+id,"720","567");
    }

    function edit(id, event) {
        stopBubble(event);
        ModifyView("修改成交案例", '/transactionCase/add?action=edit&id='+id,"720","567");
    }

    function batchDelete(ids) {
        DeletedBatch('批量删除成交案例后将无法回复,您确定执行次操作？', '/transactionCase/batchDelete',ids);
    }

    function deleted(id, event) {
        stopBubble(event);
        Deleted('成交案例删除后将无法回复,您确定执行次操作？','/transactionCase/delete',id);
    }
