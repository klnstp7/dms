...
<table id="offerCaseTable" class="cell-border nowrap" cellspacing="0" width="100%">
</table>
<div class="" style="margin-top: 20px">
    <div class="panel">
        ...
        <div class="panel-body">
            <table id="offerCaseHisTable" class="cell-border nowrap" cellspacing="0" width="100%">
            </table>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    var sourceTable;
    <%
    out.print("sourceTable="+SourceTableEmum.OfferCase.ordinal()+";");
    %>
    $(function() {
        _initTable("#offerCaseTable","/offerCase/getPaging");
        $("input").placeholder();
        ...
        $("#rowDown").click(function() {
            if (_table.rows('.selected').data().length == 0) {
                ...
                postcall("/offerCase/batchDownload", {ids: ids}, '_blank');
            }
        });
        ...
        $('#offerCaseTable tbody').on('click', 'tr', function() {
            var _obj = _table.row($(this)).data();
            _initHis("#offerCaseHisTable","/offerCase/getHis",_obj.id);
        });
    ...
    function search(){
        $('#offerCaseTable').DataTable().clear();
        $('#offerCaseTable').DataTable().destroy();
        _initTable("#offerCaseTable","/offerCase/getPaging");
    }

    function info(id, event) {
        stopBubble(event);
        ModifyView("查看报盘案例", '/offerCase/add?action=view&id='+id,"720","567");
    }

    function edit(id, event) {
        stopBubble(event);
        ModifyView("修改报盘案例", '/offerCase/add?action=edit&id='+id,"720","567");
    }

    function batchDelete(ids) {
        DeletedBatch('批量删除报盘案例后将无法回复,您确定执行次操作？', '/offerCase/batchDelete',ids);
    }

    function deleted(id, event) {
        stopBubble(event);
        Deleted('报盘案例删除后将无法回复,您确定执行次操作？','/offerCase/delete',id);
    }
