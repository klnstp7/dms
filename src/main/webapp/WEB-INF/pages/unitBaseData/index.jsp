...
<table id="unitBaseDataTable" class="cell-border nowrap" cellspacing="0" width="100%">
</table>
<div class="" style="margin-top: 20px">
    <div class="panel">
        ...
        <div class="panel-body">
            <table id="unitBaseDataHisTable" class="cell-border nowrap" cellspacing="0" width="100%">
            </table>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    %>
    $(function() {
        _initTable("#unitBaseDataTable", "/unitBaseData/getPaging");
        $("input").placeholder();
        ...
        $("#rowDown").click(function() {
            if (_table.rows('.selected').data().length == 0) {
                ...
                postcall("/unitBaseData/batchDownload", {ids: ids}, '_blank');
            }
        });
        ...
        $('#unitBaseDataTable tbody').on('click', 'tr', function() {
            var _obj = _table.row($(this)).data();
            _initHis("#unitBaseDataHisTable","/unitBaseData/getHis",_obj.id);
        });
    ...
    function search(){
        $('#unitBaseDataTable').DataTable().clear();//清空数据
        $('#unitBaseDataTable').DataTable().destroy();//销毁datatable
        _initTable("#unitBaseDataTable", "/unitBaseData/getPaging");
    }

    function info(id, event) {
        stopBubble(event);
        ModifyView( '查看单元','/unitBaseData/add?action=view&id='+id,"720","567");
    }

    function edit(id, event) {
        stopBubble(event);
        ModifyView( '修改单元','/unitBaseData/add?action=edit&id='+id,"720","567");
    }

    function batchDelete(ids) {
        DeletedBatch('批量删除单元信息后将无法回复,您确定执行次操作？', '/unitBaseData/batchDelete',ids);
    }

    function deleted(id, event) {
        stopBubble(event);
        Deleted('单元信息删除后将无法回复,您确定执行次操作？','/unitBaseData/delete',id);
    }
</script>
