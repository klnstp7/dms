 ...
 */
@Controller
@RequestMapping("/unitBaseData")
public class UnitBaseDataController {
    @Autowired
    private IUnitBaseDataService unitBaseDataService;
    @Autowired
    private IUnitBaseDataHisService unitBaseDataHisService;
    @Autowired
    private IExtendConfigService extendConfigService;

    @RequestMapping("/index")
    public ModelAndView index() {
        ModelAndView mv = new ModelAndView("/unitBaseData/index");
        return mv;
    }
    ...
    public void getPaging(@RequestParam int start, @RequestParam int length, @RequestParam int draw, @RequestParam String filter, HttpServletResponse response)
        ...
        PageVo<UnitBaseDataDto> pageVo =  unitBaseDataService.findByPaging(start,length,draw,filter,user.getCompanyId());
    ...
    public ModelAndView add() {
        ModelAndView mv = new ModelAndView("/unitBaseData/add");
        List<String> columns=new ArrayList<String>();
        Map<String, TempLateConfigDto> configColMap = XmlUtil.getConfigForInput("unitbase","entityCol");
    ...
    public Boolean update(UnitBaseDataDto dto) throws CustomException {
        UserInfoApiDto user=UserInfoUtil.getCurrentUser();
        dto.setCompanyId(user.getCompanyId());
        return unitBaseDataService.update(dto,user.getCompanyId(),user.getLoginName());
    }

    @ResponseBody
    @RequestMapping(value="delete",method = RequestMethod.POST)
    public Boolean delete(@RequestParam Long id) throws CustomException{
        return unitBaseDataService.delete(id,UserInfoUtil.getCurrentUser().getCompanyId());
    }

    @ResponseBody
    @RequestMapping(value="batchDelete",method = RequestMethod.POST)
    public Boolean batchDelete(@RequestParam Long[] ids) throws CustomException{
        return unitBaseDataService.batchDelete(java.util.Arrays.asList(ids),UserInfoUtil.getCurrentUser().getCompanyId());
    }

    @ResponseBody
    @RequestMapping(value="get",method = RequestMethod.GET)
    public UnitBaseDataDto get(@RequestParam Long id){
        return unitBaseDataService.get(id);
    }
    ...
    public void getHis(@RequestParam int start, @RequestParam int length, @RequestParam int draw, @RequestParam Long id, HttpServletResponse response)
            throws ServletException, IOException {
        PageVo<UnitBaseDataHisDto> list =  unitBaseDataHisService.findByPaging(start,length,draw,id);
    ...
    public void batchDownload(@RequestParam Long[] ids, HttpServletResponse response){
        ...
        UnitBaseDataCond.Criteria criteria=cond.createCriteria();
        criteria.andIdIn(java.util.Arrays.asList(ids));
        List<UnitBaseDataDto> list=unitBaseDataService.selectByCond(cond);
        if(list==null||list.isEmpty()){
            return;
        }
        String jsonStr=JsonUtil.getExtendJsonString(list);
        JSONArray jsonArray = JSONArray.fromObject(jsonStr);
        List<ExtendConfigDto> configs = extendConfigService.getExtendTitleList(UserInfoUtil.getCurrentUserCompanyId(), SourceTableEmum.UnitBaseData.ordinal());
        ...
        ExcelUtil.exportJsonToExcel("单元基础数据下载-"+dataFormat.format(new Date()),configColMap, jsonArray, response);
    }
}
